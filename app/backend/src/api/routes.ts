...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/providers/stratz/match-test/:matchId", { config: expensiveWriteRateLimit }, async (request, reply) => {
    if (!(await isAdminRequest(request))) {
      reply.code(403);
      return { message: "Admin access required." };
    }
    const params = z.object({ matchId: z.coerce.number().int().positive() }).parse(request.params);
    try {
      return await service.testStratzMatchTelemetry(params.matchId);
    } catch (error) {
      reply.code(400);
      return { message: error instanceof Error ? error.message : "STRATZ match telemetry test failed." };
    }
  });
